...
defmodule StatisticsInfo do
  ...
  def init(:ok) do
    # :ets.insert(@tab, {:prev, @init_prev})
    # 刷新排行榜
    # e_refresh = Parser.parse!("*/#{@per_sec}", true) #|> #IO.inspect  # 统计一次机器人数据上传
    # Scheduler.new_job()
    # |> Quantum.Job.set_schedule(e_refresh)
    # |> Quantum.Job.set_name(:statistics_info_update)
  ...
  def statistics_info_update(_num \\ 0) do
    ...
    new_prev = Enum.zip([:recv_pkg_total_num, :send_pkg_total_num, :robot_total_num, :robot_online_num], MsgCounter.take_counts()) |> Map.new()
    # IO.inspect "new_prev #{inspect new_prev}"
    # Logger.info "#{inspect new_prev}"
    data = Enum.map(prev, fn {k, v} ->
      {k, (new_prev[k] || 0) - v}
    end)
    |> Map.new
    ...
    if data[:robot_online_num] <= data[:robot_total_num] do
      ...
      WeTestApi.statistics_info(transParamsStr)
